...
import { DEMO_PROPERTIES, isDemoDataEnabled } from '@/lib/demoData';
...
const INITIAL_PROPERTIES: Property[] = isDemoDataEnabled ? DEMO_PROPERTIES : [];
...
export default function MapPageClient() {
    ...
    // What the user asked for before being sent through authentication
    const [pendingAction, setPendingAction] = useState<'list' | null>(null);
    ...
    // The URL is the single source of truth for filters, so searches coming from
    // the landing page are applied and browser navigation restores them.
    ...
    useEffect(() => {
        async function loadProperties() {
            try {
                ...
                setProperties(isDemoDataEnabled ? [...firestoreProps, ...DEMO_PROPERTIES] : firestoreProps);
            } catch (err) {
    ...
    const handleInitiateListing = useCallback(() => {
        if (!user) {
    ...
    // Entering the map with ?action=list starts the listing flow once auth is known
    const listIntentHandled = useRef(false);
    useEffect(() => {
        if (initializing || listIntentHandled.current) return;
        if (searchParams.get('action') !== 'list') return;

        listIntentHandled.current = true;
        if (user) {
            startDrawing();
    ...
    }, [initializing, user, searchParams, filters, router, startDrawing]);
    ...
    const handleAddPropertySuccess = async () => {
        try {
            ...
            setProperties(isDemoDataEnabled ? [...updated, ...DEMO_PROPERTIES] : updated);
        } catch (e) {
            console.error(e);
        }
    };
    ...
    return (
        <main className="relative flex h-screen w-full overflow-hidden bg-slate-50 text-slate-900 dark:bg-[#0b0d10] dark:text-gray-100 transition-colors duration-200">
            ...
            <AuthModal
                ...
                intent={pendingAction === 'list' ? 'list' : 'signin'}
                onSuccess={() => {
