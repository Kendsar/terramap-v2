  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[2000] flex max-h-[90vh] w-full max-w-[540px] translate-x-[-50%] translate-y-[-50%] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white text-slate-900 dark:border-white/10 dark:bg-[#15181e] dark:text-gray-100 shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
          
          {/* Header */}
          <div className="relative border-b border-slate-200 dark:border-white/10 px-6 py-5 shrink-0 bg-gradient-to-r from-emerald-500/10 via-transparent to-transparent">
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                  <Lock className="h-5 w-5 stroke-[2.2px]" />
                </div>
                <div>
                  <Dialog.Title className="text-lg font-bold text-slate-900 dark:text-gray-100 flex items-center gap-2">
                    {title}
                  </Dialog.Title>
                  <Dialog.Description className="mt-1 text-xs text-slate-500 dark:text-gray-400 leading-relaxed max-w-sm">
                    {description}
                  </Dialog.Description>
                </div>
              </div>
            ...
            {activeTab !== 'forgot' && (
              <div className="mt-4 flex rounded-xl border border-slate-200 bg-slate-100 dark:border-white/10 dark:bg-white/5 p-1">
                <button
                  type="button"
          ...
          {/* Footer note */}
          <div className="border-t border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/[0.02] px-6 py-3 shrink-0 flex items-center justify-between text-[11px] text-slate-500 dark:text-gray-400">
            ...
            <a
              ...
              className="flex items-center gap-1 text-slate-500 hover:text-emerald-600 dark:text-gray-400 dark:hover:text-emerald-400 transition-colors"
